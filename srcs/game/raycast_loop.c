...
void	raycast_init(t_ray *r, int ray_no, const t_game *g)
{
	...
	r->hori.step_size = fabs(1. / r->ray_dir.x);//sqrt(1 + pow(r->ray_dir.y / r->ray_dir.x, 2.));
	r->verti.step_size = fabs(1. / r->ray_dir.y);//sqrt(1 + pow(r->ray_dir.x / r->ray_dir.y, 2.));
...
double	raycast_fin(t_ray *r, const t_ply *p)
{
	...
	if (r->side == horizontal)
	{
		...
		perp_dist = r->hori.check_dist - r->hori.step_dir;
		if (r->ray_dir.y < 0)
			r->side = west;
		else
			r->side = east;
	...
	// perp_dist = r->verti.check_dist - p->pos.x + ((1 - r->verti.step_dir) / 2);
	perp_dist = r->verti.check_dist - r->verti.step_dir;
	if (r->ray_dir.x < 0)
		r->side = north;
	else
		r->side = south;
